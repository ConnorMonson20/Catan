...
export function handleBuild(
  ...
  if (buildType === 'settlement') {
    ...
    const free = inSetup;
    const cost: ResourceCounts = { brick: 1, lumber: 1, wool: 1, grain: 1, ore: 0, gold: 0, water: 0 };
    ...
    const cost: ResourceCounts = { brick: 0, lumber: 0, wool: 0, grain: 2, ore: 3, gold: 0, water: 0 };
    ...
    const cost: ResourceCounts = { brick: 1, lumber: 1, wool: 0, grain: 0, ore: 0, gold: 0, water: 0 };
    if (!free && !canAfford(player, cost)) return 'Not enough resources.';
...
export function handleBankTrade(
  ...
  const active = state.players[state.currentPlayerIndex];
  if (!active || active.id !== playerId) return 'Not your turn.';
  if (active.resources[give] < ratio) return 'Not enough resources to trade.';
  active.resources[give] -= ratio;
  active.resources[get] += 1;
  addLog(state, `${active.name} traded ${ratio} ${give} for 1 ${get}.`);
  return null;
}
