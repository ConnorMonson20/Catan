...
const RESOURCE_TEXTURE: Partial<Record<ResourceType | "desert", string>> = {
  ...
  lumber: "/tiles/wood.png",
  wool: "/tiles/sheep.png",
  grain: "/tiles/wheat.png",
  ore: "/tiles/ore.png",
  desert: "/tiles/desert.png",
  gold: "/tiles/gold.png",
  water: "/water.png", // keep water as texture
};

const RESOURCE_ICON: Record<ResourceType | "desert", string> = {
  brick: "B",
  lumber: "W",
  wool: "S",
  grain: "H",
...
const PROBABILITY_PIPS: Record<number, string> = {
  2: ".",
  3: "..",
  4: "...",
  5: "....",
...
export default function App() {
    ...
    socket.onmessage = (event) => {
      ...
      if (message.type === "state") {
        ...
        if (message.state.phase === "lobby") {
          setMapDraft(message.state.board.hexes.map((h) => ({ id: h.id, resource: "desert", numberToken: undefined })));
        } else {
          setMapPage(false);
        }
  ...
  const editorGrid = useMemo(() => {
    ...
    for (let q = -EDITOR_RADIUS; q <= EDITOR_RADIUS; q++) {
      for (let r = -EDITOR_RADIUS; r <= EDITOR_RADIUS; r++) {
        const s = -q - r;
        if (Math.max(Math.abs(q), Math.abs(r), Math.abs(s)) <= EDITOR_RADIUS) {
          coords.push({ q, r });
        }
      }
    }
  ...
  // No outer water tiles; a single background image sits behind the board.
  const waterFrame: { id: string; x: number; y: number }[] = [];
          ...
          <div className="map-page-bar">
            ...
            <div className="map-controls inline">
              <select value={editResource} onChange={(e) => setEditResource(e.target.value as ResourceType | "desert")}>
                {(["brick", "lumber", "wool", "grain", "ore", "gold", "water", "desert"] as Array<ResourceType | "desert">).map((r) => (
              ...
              <button onClick={() => setMapPage(false)}>Exit</button>
            </div>
          </div>
            ...
            >
              <defs>
                <pattern
                  ...
                  height="1"
                >
                  <image href="./water.png" x="0" y="0" width="1" height="1" preserveAspectRatio="xMidYMid slice" />
                </pattern>
                ...
                <linearGradient id="water-stroke" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#dff4ff" />
                  <stop offset="100%" stopColor="#4ca3ff" />
                </linearGradient>
              </defs>
              {boardRing && <polygon points={boardRing.points} fill="#0b5d96" stroke="#0c3f6d" strokeWidth={12} opacity={0.9} />}
              {state.board.hexes.map((hex) => {
                const override = mapDraft.find((h) => h.id === hex.id);
                const resource = override?.resource ?? hex.resource;
                const numberToken = override?.numberToken ?? hex.numberToken;
                return (
                  <g key={hex.id} onClick={() => handleHexClick(hex.id)} className="hex">
                    <polygon
                      ...
                      stroke={resource === "water" ? "url(#water-stroke)" : "#f5e0b3"}
                      strokeWidth={resource === "water" ? 0 : 5}
                      className="hex-bg"
                    />
                    <text x={hex.x} y={hex.y - 12} textAnchor="middle" className="hex-icon">
                    ...
                    {numberToken && (
                      <g>
                        ...
                        />
                        <text x={hex.x} y={hex.y + 4} textAnchor="middle" className={`token ${numberToken === 6 || numberToken === 8 ? 'hot' : ''}`}>
                          {numberToken}
                        </text>
                        <text x={hex.x} y={hex.y + 14} textAnchor="middle" className="token-pips">
                          {PROBABILITY_PIPS[numberToken] || ""}
                        </text>
                      </g>
                    )}
                  </g>
        ...
        <div className="main-layout">
                      ...
                      <polygon
                        ...
                        fill={
                          RESOURCE_TEXTURE[hex.resource as ResourceType | "desert"]
                            ...
                            : hex.resource === "water"
                              ? "url(#water-texture)"
                              : RESOURCE_COLOR[hex.resource]
                        }
                        stroke={hex.resource === "water" ? "#d8cbb3" : "#f5e0b3"}
                        strokeWidth={hex.resource === "water" ? 0 : 5}
                      ...
                      {hex.numberToken && (
                        <g>
                          <rect
                            ...
                            className="token-rect"
                          />
                          <text x={hex.x} y={hex.y + 4} textAnchor="middle" className={`token ${hex.numberToken === 6 || hex.numberToken === 8 ? 'hot' : ''}`}>
                            {hex.numberToken}
                          </text>
                          <text x={hex.x} y={hex.y + 14} textAnchor="middle" className="token-pips">
                            {PROBABILITY_PIPS[hex.numberToken] || ""}
                          </text>
                        </g>
                      )}
                    </g>
                  ...
                  {state.board.edges.map((edge) => {
                    ...
                    return (
                      <line
                        ...
                        y2={v2.y}
                        stroke={color}
                        strokeWidth={ownerId ? 10 : highlight ? 9 : 4}
                        opacity={ownerId ? 0.9 : highlight ? 0.85 : 0.55}
          ...
          <div className="side-column">
            ...
            <div className="panel bank-panel">
              <div className="panel-head">
                <h3>Dev Cards</h3>
                <span className="pill small">Deck: {state?.devDeck.length ?? 0}</span>
              </div>
              <div className="dev-mini-grid">
                {devCounts.knight > 0 && (
                  <div className="dev-mini-card">
                    <div className="dev-mini-top">
                      <span className="dev-label">{DEV_ICONS.knight} Knight</span>
                      <span className="count">{devCounts.knight}</span>
                    </div>
                    <button
                      onClick={() => {
                      ...
                      disabled={!isMyTurn || me?.devPlayedThisTurn}
                    >
                      Play
                    </button>
                  </div>
                )}
                {devCounts.monopoly > 0 && (
                  <div className="dev-mini-card">
                    <div className="dev-mini-top">
                      <span className="dev-label">{DEV_ICONS.monopoly} Monopoly</span>
                      <span className="count">{devCounts.monopoly}</span>
                    </div>
                    ...
                    <button onClick={() => send({ type: "playMonopoly", resource: monoResource })} disabled={!isMyTurn || me?.devPlayedThisTurn}>
                      Play
                    </button>
                  </div>
                )}
                {devCounts.year_of_plenty > 0 && (
                  <div className="dev-mini-card">
                    <div className="dev-mini-top">
                      <span className="dev-label">{DEV_ICONS.year_of_plenty} Year of Plenty</span>
                      <span className="count">{devCounts.year_of_plenty}</span>
                    </div>
                    <div className="dev-mini-row">
                    ...
                    <button onClick={() => send({ type: "playYearOfPlenty", resourceA: yopA, resourceB: yopB })} disabled={!isMyTurn || me?.devPlayedThisTurn}>
                      Play
                    </button>
                  </div>
                )}
                {devCounts.road_building > 0 && (
                  <div className="dev-mini-card">
                    <div className="dev-mini-top">
                      <span className="dev-label">{DEV_ICONS.road_building} Road Building</span>
                      <span className="count">{devCounts.road_building}</span>
                    </div>
                    <button
                      onClick={() => {
                    ...
                    {me?.bonusRoads ? <div className="pill small">Free roads: {me.bonusRoads}</div> : null}
                  </div>
                )}
                {devCounts.victory_point > 0 && (
                  <div className="dev-mini-card">
                    <div className="dev-mini-top">
                      <span className="dev-label">{DEV_ICONS.victory_point} Victory Points</span>
                    </div>
                    <div className="pill small">Held: {devCounts.victory_point}</div>
                  </div>
                )}
              </div>
            </div>
