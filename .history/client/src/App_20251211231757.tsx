...
export default function App() {
  ...
  >([]);
  const [editorRadius, setEditorRadius] = useState<number>(DEFAULT_EDITOR_RADIUS);
      ...
      const message = JSON.parse(event.data) as ServerMessage;
        if (message.type === "state") {
          ...
          if (message.state.phase === "lobby") {
            setMapDraft(message.state.board.hexes.map((h, idx) => ({
              id: h.id || `hex-${idx}`,
              q: h.q,
              r: h.r,
              x: h.x,
  ...
  const editorGrid = useMemo(() => {
    ...
    for (let q = -editorRadius; q <= editorRadius; q++) {
      for (let r = -editorRadius; r <= editorRadius; r++) {
        const s = -q - r;
        if (Math.max(Math.abs(q), Math.abs(r), Math.abs(s)) <= editorRadius) {
          coords.push({ q, r });
        }
      }
    }
            ...
            <div className="map-controls inline">
                <label style={{display:'inline-flex',alignItems:'center',gap:8}}>
                  ...
                  <input
                    ...
                    max={30}
                    value={editorRadius}
                    onChange={(e) => setEditorRadius(Math.max(1, Math.min(30, Number(e.target.value) || 1)))}
