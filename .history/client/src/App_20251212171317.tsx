...
export default function App() {
  ...
  const [cityColor, setCityColor] = useLocalStorage('catan-city-color', '#ffd166');
  const [cityColor2, setCityColor2] = useLocalStorage('catan-city-color-2', '#ffb703');
  const [roadColor, setRoadColor] = useLocalStorage('catan-road-color', '#8b5a2b');
  ...
  useEffect(() => {
    try {
      document.documentElement.style.setProperty('--city-color', cityColor || '#ffd166');
      document.documentElement.style.setProperty('--city-color-2', cityColor2 || '#ffb703');
      document.documentElement.style.setProperty('--road-color', roadColor || '#8b5a2b');
    } catch (err) {
                  ...
                  {state.board.edges.map((edge) => {
                    ...
                    return (
                      <line
                        ...
                        x2={v2.x}
                        y2={v2.y}
                        stroke={ownerId ? 'var(--road-color)' : '#2f3542'}
                  ...
                  {state.board.vertices.map((vertex) => {
                    ...
                    return (
                      <g key={vertex.id} onClick={() => handleVertexClick(vertex.id)} className="vertex">
                        ...
                        />
                        {isCity && <circle cx={vertex.x} cy={vertex.y} r={8} fill="var(--city-color-2)" opacity={0.95} />}
                      </g>
                    );
                  })}
            ...
            <div className="panel">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ width: 120 }}>City icon color</span>
                  <input type="color" value={cityColor} onChange={(e) => setCityColor(e.target.value)} />
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ width: 120 }}>Placed city color</span>
                  <input type="color" value={cityColor2} onChange={(e) => setCityColor2(e.target.value)} />
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ width: 120 }}>Road color</span>
